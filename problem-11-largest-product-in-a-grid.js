...
function largestGridProduct(arr) {
  // Good luck!
  // We are only checking from current position right, down, down right, down left
  const DIRECTIONS = [[1, 0], [0, 1], [-1, 1], [1, 1]];
  const collect = (arr, x, y, [xDIR, yDIR], n = 4) => {
    let result = [];
    for (let i = 0; i < n; i++) {
      if (arr[y] === undefined || arr[y][x] === undefined) continue;
      result.push(arr[x][y]);
      x += xDIR;
      y += yDIR;
    }
    ...
    if (result.length === 4) {
      return result.reduce((acc, i) => acc * i, 1);
    }
    else return 0;
  };
  ...
  for (let y = 0; y < arr.length; y++) {
    for (let x = 0; x < arr[y].length; x++) {
      let test = DIRECTIONS.map(i => collect(arr, x, y, i));
      let m = Math.max(...test);
      largestFound = m > largestFound ? m : largestFound;
    }
  }
